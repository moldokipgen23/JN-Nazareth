        ...
        <div style="background:#fff;border-radius:12px;padding:14px;margin-bottom:10px;box-shadow:0 1px 3px rgba(15,23,42,.06);">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:6px;">
                @foreach($exams as $exam)
                    @php
                        $s = $slotStatuses[$slot->class][$slot->section][$slot->subject][$exam->id] ?? null;
                    @endphp
                    <div style="display:flex;align-items:center;gap:4px;">
                        ...
                        @if($s)
                            <span style="background:{{ $s['bg'] }};color:{{ $s['color'] }};font-size:10px;font-weight:700;padding:3px 9px;border-radius:99px;">{{ $s['icon'] }} {{ $s['label'] }}</span>
                        @else
