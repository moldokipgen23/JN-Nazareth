...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    ...
    <a href="{{ route('admin.hall-of-fame.create') }}"
...
<div style="display:flex; flex-direction:column; gap:12px;" id="firsts-list">
    ...
    <div data-id="{{ $item->id }}" style="background:#fff; border-radius:14px; box-shadow:0 1px 8px rgba(0,0,0,.06); border:1px solid {{ $item->active ? '#f1f5f9' : '#fef2f2' }}; padding:16px 20px; display:flex; align-items:center; gap:16px;">
        ...
        <div style="flex-shrink:0; width:56px; height:56px; border-radius:10px; overflow:hidden; background:#f1f5f9; border:1px solid #e2e8f0;">
            @if($item->photo)
            <img src="{{ \App\Helpers\Settings::storageUrl($item->photo) }}" alt="{{ $item->name }}" style="width:100%; height:100%; object-fit:cover;">
            @else
        ...
        <div style="flex:1; min-width:0;">
            <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap; margin-bottom:3px;">
                <span style="font-size:14px; font-weight:700; color:#0f172a;">{{ $item->name }}</span>
                @if($item->featured)
            ...
            <div style="font-size:12px; color:#0f766e; font-weight:600;">{{ $item->achievement_title }}</div>
            @if($item->year)
            <div style="font-size:11px; color:#94a3b8; margin-top:2px;">{{ $item->year }}</div>
            @endif
            @if($item->description)
            <div style="font-size:11px; color:#64748b; margin-top:4px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:400px;">{{ $item->description }}</div>
            @endif
        </div>

        {{-- Actions --}}
        <div style="display:flex; gap:6px; flex-shrink:0;">
            <a href="{{ route('admin.hall-of-fame.edit', $item) }}"
            ...
            <form method="POST" action="{{ route('admin.hall-of-fame.toggle', $item) }}" style="display:inline;">
                @csrf
                <button type="submit"
                        style="display:inline-flex; align-items:center; gap:5px; background:{{ $item->active ? '#fef9c3' : '#f0fdf4' }}; color:{{ $item->active ? '#854d0e' : '#15803d' }}; border:1px solid {{ $item->active ? '#fde68a' : '#bbf7d0' }}; padding:6px 12px; border-radius:8px; font-size:12px; font-weight:600; cursor:pointer;">
                    {{ $item->active ? 'Hide' : 'Show' }}
                </button>
            </form>
            <form method="POST" action="{{ route('admin.hall-of-fame.destroy', $item) }}"
...
(function() {
    ...
    function saveOrder() {
        ...
        fetch('{{ route("admin.hall-of-fame.reorder") }}', {
            method: 'POST',
            headers: {
