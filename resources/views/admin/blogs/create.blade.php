...
<div class="max-w-3xl">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-xl font-bold text-gray-900">New Blog Post</h2>
            <p class="text-sm text-gray-500 mt-0.5">Write and publish a new post.</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.blogs.store') }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="14" required
                          ...
                          class="w-full px-3 py-2 border @error('content') border-red-400 @else border-gray-300 @enderror rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none resize-y">{{ old('content') }}</textarea>
                @error('content')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
