...
<div style="display:flex; align-items:center; gap:6px; font-size:13px; margin-bottom:20px;">
    ...
    <a href="{{ route('admin.folders.show', $folder) }}" style="color:#0d9488; text-decoration:none; font-weight:500;">{{ $folder->name }}</a>
    <svg width="14" height="14" fill="none" stroke="#cbd5e1" stroke-width="2" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"/></svg>
    <span style="color:#0f172a; font-weight:700;">Edit</span>
</div>

<div style="max-width:480px;">
    <div style="background:#fff; border-radius:14px; border:1px solid #f1f5f9; box-shadow:0 1px 8px rgba(0,0,0,.06); padding:24px;">
        <div style="font-size:14px; font-weight:700; color:#0f172a; margin-bottom:18px;">Rename Folder</div>

        @if(session('success'))
        <div style="background:#f0fdf4; border:1px solid #bbf7d0; color:#166534; padding:11px 16px; border-radius:9px; font-size:13px; margin-bottom:16px;">
            {{ session('success') }}
        </div>
        @endif
        ...
        <form method="POST" action="{{ route('admin.folders.update', $folder) }}">
            ...
            <div style="margin-bottom:16px;">
                ...
                <input type="text" name="name" value="{{ old('name', $folder->name) }}" required autofocus
            ...
            <div style="background:#f8fafc; border-radius:9px; padding:10px 14px; font-size:12px; color:#64748b; margin-bottom:16px; display:flex; justify-content:space-between;">
                <span>Files</span><strong style="color:#0f172a;">{{ $folder->files()->count() }}</strong>
            </div>
