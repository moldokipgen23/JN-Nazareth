        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <select id="role" name="role" required onchange="toggleClassAccess()"
                    ...
                    <option value="staff"   {{ old('role') === 'staff'   ? 'selected' : '' }}>Staff — manage content</option>
                    <option value="teacher" {{ old('role') === 'teacher' ? 'selected' : '' }}>Teacher — view assigned classes</option>
            ...
            <div id="classAccess" style="display:none;">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    ...
                    <label class="flex items-center gap-2 text-sm bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 cursor-pointer">
                        <input type="checkbox" name="classes[]" value="{{ $c }}" {{ in_array($c, old('classes', [])) ? 'checked' : '' }}>
                        {{ $c }}
                    </label>
                    @endforeach
                </div>
