...
<section style="background:#111; width:100%; overflow:hidden; line-height:0; position:relative; font-size:0;">
    ...
    <div style="position:absolute; bottom:0; left:0; right:0; z-index:10; padding:24px 32px; background:linear-gradient(to top, rgba(0,0,0,.75) 0%, transparent 100%); pointer-events:none;">
        @if($storyVideoTitle)
        <h2 style="font-family:'Playfair Display',serif; font-size:clamp(1.2rem,2.5vw,2rem); font-weight:800; color:#fff; margin:0 0 6px; text-shadow:0 2px 8px rgba(0,0,0,.5);">{{ $storyVideoTitle }}</h2>
        @endif
        @if($storyVideoSub)
        <p style="color:rgba(255,255,255,.8); font-size:.95rem; margin:0; text-shadow:0 1px 4px rgba(0,0,0,.5);">{{ $storyVideoSub }}</p>
        @endif
    </div>
    @endif
    ...
    <div style="position:relative; width:100%; padding-top:56.25%; overflow:hidden;">
        <iframe
            src="https://www.youtube.com/embed/{{ $ytVid }}?autoplay=1&mute=1&loop=1&playlist={{ $ytVid }}&controls=0&showinfo=0&rel=0&iv_load_policy=3&modestbranding=1&playsinline=1&disablekb=1&fs=0&enablejsapi=0"
