    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div style="text-align:center; margin-bottom:52px;">
            ...
            <h2 style="font-family:'Playfair Display',serif; font-size:clamp(1.8rem,3.5vw,2.8rem); font-weight:800; color:#1c1917; margin:12px 0 12px; line-height:1.2;">
                {{ \App\Helpers\Settings::get('sec_title_location', 'How to Find Us') }}
            </h2>
            @if(\App\Helpers\Settings::get('map_section_subtitle'))
            <p style="color:#78716c; font-size:.95rem; max-width:500px; margin:0 auto; line-height:1.7;">
                {{ \App\Helpers\Settings::get('map_section_subtitle') }}
            </p>
            @endif
        </div>
            ...
            <div>
                <div style="background:#fff; border-radius:20px; padding:28px; box-shadow:0 4px 24px rgba(0,0,0,.08); border:1px solid #e7dfd4; margin-bottom:24px;">
                    ...
                    <div style="position:relative;">
                        @if(count($routeSteps) > 1)
                        <div style="position:absolute; left:18px; top:24px; bottom:24px; width:2px; background:linear-gradient(to bottom, {{ $routeSteps[0]['color'] ?? '#2d6a4f' }}, {{ $routeSteps[count($routeSteps)-1]['color'] ?? '#166534' }}); border-radius:2px; z-index:0;"></div>
                        @endif
                        ...
                        <div style="display:flex; flex-direction:column; gap:0;">
                            ...
                            <div style="display:flex; align-items:center; gap:16px; padding:10px 0; position:relative; z-index:1;">
                                <div style="width:38px; height:38px; border-radius:50%; background:{{ $step['color'] ?? '#2d6a4f' }}; border:3px solid #fff; box-shadow:0 0 0 2px {{ $step['color'] ?? '#2d6a4f' }}40, 0 4px 12px rgba(0,0,0,.15); display:flex; align-items:center; justify-content:center; font-size:16px; flex-shrink:0; z-index:2;">
                                    {{ $step['icon'] ?? '📍' }}
                                </div>
                                <div style="flex:1; min-width:0;">
                                    <div style="font-weight:800; font-size:.95rem; color:#1c1917;">{{ $step['place'] ?? '' }}</div>
                                    <div style="font-size:.75rem; color:#78716c; margin-top:1px;">{{ $step['note'] ?? '' }}</div>
                                </div>
                                <div style="font-size:.75rem; font-weight:700; color:{{ $step['color'] ?? '#2d6a4f' }}; background:{{ $step['color'] ?? '#2d6a4f' }}18; padding:3px 10px; border-radius:20px; white-space:nowrap; flex-shrink:0;">
                                    {{ $step['dist'] ?? '' }}
                                </div>
                            </div>
                            @endforeach
                ...
                <div style="display:flex; gap:12px; flex-wrap:wrap;">
                    <a href="{{ $locMapLink }}" target="_blank" rel="noopener"
            ...
            <div>
                <div style="border-radius:18px; overflow:hidden; box-shadow:0 6px 28px rgba(0,0,0,.12); border:1px solid #e7dfd4; margin-bottom:24px;">
                    <iframe
                        ...
                        title="{{ \App\Helpers\Settings::get('sec_title_location', 'How to Find Us') }}">
                    </iframe>
                </div>

                @if(count($nearbyPlaces))
                ...
                <div style="display:flex; flex-direction:column; gap:10px;">
                    ...
                    <div style="background:#fff; border-radius:14px; padding:14px 16px; border:1px solid #e7dfd4; display:flex; align-items:center; gap:14px; box-shadow:0 2px 10px rgba(0,0,0,.05);">
                        ...
                        <div style="flex:1; min-width:0;">
                            <div style="font-weight:700; font-size:.88rem; color:#1c1917;">{{ $place['name'] ?? '' }}</div>
                            <div style="font-size:.75rem; color:#78716c; line-height:1.4; margin-top:2px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ $place['desc'] ?? '' }}</div>
                        </div>
                        @if(!empty($place['link']))
