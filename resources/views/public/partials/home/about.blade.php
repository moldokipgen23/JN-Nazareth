...
<section style="padding:80px 0; background:var(--cream);" class="scroll-reveal">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            {{-- Image collage from local photos --}}
            <div style="position:relative; height:440px;">
                ...
                <div style="position:absolute; bottom:28px; left:12px; z-index:10; background:var(--primary); color:#fff; padding:14px 18px; border-radius:14px; box-shadow:0 8px 24px rgba(0,0,0,0.25); text-align:center;">
                    ...
                    <div style="font-size:1.4rem; font-weight:700;">{{ date('Y') }}</div>
                    <div style="font-size:0.65rem; text-transform:uppercase; letter-spacing:0.1em; opacity:0.75; margin-top:2px;">{{ \App\Helpers\Settings::get('site_name','Our Community') }}</div>
                </div>
            </div>

            {{-- Text --}}
            <div>
                <span class="section-label">{{ $aboutLabel }}</span>
