<?php $__env->startSection('title', $galleryFolder->name . ' — Gallery'); ?>
...
<section class="page-hero" style="min-height:220px;">
    <div style="position:relative; z-index:2; max-width:900px; margin:0 auto; padding:56px 24px 44px; text-align:center;">
        <a href="<?php echo e(route('gallery')); ?>" style="display:inline-flex; align-items:center; gap:6px; color:rgba(255,255,255,.65); font-size:12px; font-weight:600; text-decoration:none; margin-bottom:18px; text-transform:uppercase; letter-spacing:.08em; transition:color .2s;"
        ...
        <div style="display:inline-flex; align-items:center; gap:6px; background:rgba(255,255,255,.12); backdrop-filter:blur(6px); border:1px solid rgba(255,255,255,.2); border-radius:50px; padding:6px 16px; font-size:11px; font-weight:700; color:rgba(255,255,255,.85); text-transform:uppercase; letter-spacing:.08em;">
            <?php echo e($images->total()); ?> Photos
        </div>
    </div>
</section>
...
<section style="background:var(--cream); padding:56px 0 80px;">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div id="gallery-grid" style="columns:2; column-gap:14px;" class="sm:columns-3 lg:columns-4">
            <?php $__currentLoopData = $images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="gallery-item"
                 data-src="<?php echo e(\App\Helpers\Settings::storageUrl($image->path)); ?>"
                 data-caption="<?php echo e($image->title ?? ''); ?>"
                ...
                <img src="<?php echo e(\App\Helpers\Settings::storageUrl($image->path)); ?>"
                     alt="<?php echo e($image->title ?? $galleryFolder->name); ?>"
                     loading="<?php echo e($idx < 8 ? 'eager' : 'lazy'); ?>"
                ...
                <div class="gi-ov" style="position:absolute; inset:0; background:linear-gradient(to top,rgba(27,67,50,.8) 0%,transparent 55%); opacity:0; transition:opacity .3s; display:flex; flex-direction:column; justify-content:flex-end; padding:14px;">
                    <?php if($image->title): ?>
                        <p style="color:#fff; font-family:'Playfair Display',serif; font-size:.88rem; font-weight:600; margin:0 0 4px;"><?php echo e($image->title); ?></p>
                    <?php endif; ?>
                ...
                <div style="position:absolute; top:8px; right:8px; background:rgba(0,0,0,.45); color:#fff; font-size:10px; font-weight:700; padding:2px 7px; border-radius:16px;"><?php echo e($idx + 1); ?></div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>

        <?php if($images->hasPages()): ?>
        <div style="margin-top:40px; display:flex; justify-content:center;"><?php echo e($images->links()); ?></div>
        <?php endif; ?>
    </div>
</section>
...
<section style="padding:80px 24px; text-align:center; background:var(--cream);">
    ...
    <a href="<?php echo e(route('gallery')); ?>" style="display:inline-block; margin-top:16px; color:var(--primary); font-size:.85rem; font-weight:600; text-decoration:none;">← Back to Gallery</a>
</section>
<?php endif; ?>
...
<?php echo $__env->make('layouts.public', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/moldokipgen/Projects/EHLOM DIGITAL/Ehlom CMS/resources/views/public/gallery-folder.blade.php ENDPATH**/ ?>